vec4 effect(vec4 color, Image texture, vec2 texture_coords, vec2 screen_coords) {
    ...
    // Constants
    const float baseLine = 22.0; // Pixels from top of sprite where color changes
    const float spriteHeight = 32.0; // Height of the sprite in pixels
    const float spriteWidth = 32.0; // Width of the sprite in pixels
    const float sheetHeight = 640.0; // Total height of the spritesheet in pixels
    const float sheetWidth = 320.0; // Total width of the spritesheet in pixels
    const float angle = 60.0; // Shadow projection angle in degrees
    const float PI = 3.14159265359;
    const float rad = angle * PI / 180.0;
    const vec2 spriteAngleMove = vec2(cos(rad) / spriteWidth, sin(rad) / spriteHeight); // Movement per pixel in sprite space
    const float shadowSize = 16.0; // Max shadow distance in pixels
    const float stepMove = 1.0; // Step size in pixels
    const float divideBy = 1.0; // From main.lua, adjust as needed
    ...
    if (current_pixel.a > 0.0) {
        // Normalize texture_coords.y to the sprite's local UV space (0 to 1 within sprite)
        float localUVY = fract(texture_coords.y / spriteUVHeight); // 0 to 1 within sprite
        float localPixelY = localUVY * spriteHeight; // 0 to 32 pixels within sprite
        
        // Apply white/red coloring based on localPixelY
        if (localPixelY < baseLine) {
            // Above baseLine: white
            return vec4(1.0, 1.0, 1.0, current_pixel.a);
        } else {
            // Below baseLine: red
            return vec4(1.0, 0.0, 0.0, current_pixel.a);
        }
    }
    ...
    for (int i = 0; i <= int(shadowSize); i += int(stepMove)) {
        ...
        if (posLocalPixelY >= baseLine - tolerance && posLocalPixelY <= baseLine + tolerance) {
            ...
            if (new_pixel.a > 0.0) {
                return vec4(0.0, 0.0, 0.0, 0.5); // Semi-transparent black shadow
            }
