...
vec4 effect(vec4 color, Image texture, vec2 textureCoords, vec2 screenCoords) {
    number dist = distance(textureCoords, normalizedSpotlight);                             //Calculate the distance to the center of spotlight
    
    vec4 objectPixel = Texel(objectCanvas,textureCoords);                                   //Check if object in pixel location. 
    if (objectPixel.a != 1.0) {                                                             //If objectCanvas is not opaque...
